...
\begin{figure}[H]
  ...
  \caption{
    ...
    used by Tishby\cite{TISHBY}, hence entropy of the input -- $H(X) = 12$ and
    entropy of the label -- $H(Y) = 1$.
    Some Nodes are labeled $T_{e,l}$, where $e$ is the epoch number and $l$ is
    the layer the node belongs to. Consider $T_{1,3}$ from Figure (b) -- it is
    the node for layer 3 and epoch one, the $x-axis$ value $\approx{8}$ is $I(X,
    T_{1,3})$, while the $y-axis$ value $\approx{0.1}$ is $I(Y, T_{1,3})$ 
  }
\end{figure}
