...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.60\textwidth]{figs/ip_1v3.png}
  \caption{
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.60\textwidth]{figs/ip_10000v3.png}
  \captionof{figure}{
    Information plane for a neural network with 4 layers, which was trained for
    approximately $10\,000$ epochs. Some Nodes are labeled $T_{e,l}$, where $e$
    is the epoch number and $l$ is the layer the node belongs to. Consider
    $T_{1,3}$ it's the node for layer 3 and epoch one, the $x-axis$ value
    $\approx{8}$ is $I(X, T_{1,3})$, while the $y-axis$ value $\approx{0.1}$ is
    $I(Y, T_{1,3})$
    Information plane for a neural network with 4 layers, which was trained for
