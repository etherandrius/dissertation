...
\autoref{eq:layerL} -- we will need to get access to intermediate layers in our
experiments.
...
what information the NN is retaining through out the layers, and how the
information retained changes through out the training process.
...
$X,T_{e,t},Y$. Given the probability distribution we now have needed MI values:
$I(T_{e,t}, X)$ and $I(T_{e,t}, Y)$.
...
\begin{figure}[H]
    ...
    \end{pythonfigure}
    \caption{
      Definition of correlated random variables $X, T_{e,t}$ and, $Y$. The
      Probability distributions are generated using the dataset
      $(x_i, y_i)$ for $i = 1,...,N$, with the assumption that every input is
      equally likely.
    }
    \label{fig:rxty}
\end{figure}
...
Consider \autoref{fig:rxty2}, the routine defines the random variables
...
\begin{itemize}
  \item{
      A slowdown of how fast $I(Y, T_{e,t})$ is increasing with respect to epoch
      $e$. 
    }
  \item{
