...
\begin{figure}[H]
    \begin{pythonfigure}
      ...
      # Input:
      # Data  - Training data $D$ defined as $(x_i, y_i)$ for $i=1,..,N$,
...
\begin{itemize}
  \item{
      Recall \autoref{subIPsetup} It talks about how we can measure information
      within NN.
    }
  \item{
      We note that in order to measure mutual information we need a probability
      distributions.
    }
  \item{
      Input:
      \begin{itemize}
        \item{
            We have data set $D$ which is: $(x_i, y_i)$ for $i=1,...,N$. Let
            $D_x=\{x_1,...,x_N\}$, $D_x=\{y_1,...,y_N\}$
          }
        \item{
            Which is a subset of all possible $(x_i, y_i)$ pairs, which could be
            infinite.
          }
        \item{
            We train our NN on a subset of $D$. 
          }
        \item{
            We measure the NNs information retention on the $D$
          }
      \end{itemize}
    }
  \item{
      However, we only have a discrete empirical sample ${}$
    }
  \item{
      We note that in order to measure mutual information we need probability
      distributions, which we generate via routine described in
      \autoref{fig:rxty}. Given the results of \autoref{subDTF} we need to
      redefine the results as in \autoref{figRxty2}.
    }
  \item{
      \autoref{figRxty2} defines correlated random variables: $X,T_{e,l},Y$ --
      and hence the MI values: $I(X,T_{e,l})$ and $I(Y,T_{e,l})$.
    }
  \item{
      However, computing the MI values exactly is a computationally infeasible
      task -- as we cannot capture the full randomness produced by
      \monospace{line 2}.
    }
  \item{
      As such we need a way to estimate the MI values. Fundamentally we need
      make assumptions about the Input distributions that allows us to
      efficiently compute the values. 
    }
  \item{
      We will consider 3 different MIE techniques: Discrete, KDE, AIR -- all of
      which simulate randomness in a different way.
    }
\end{itemize}
