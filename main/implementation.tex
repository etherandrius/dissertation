...
\autoref{figGetProbabilities} is a helper function used by Mutual Information
Estimators subsections \ref{subBinning} and \ref{subKDE}.
...
\subsection{MIE - Binning (Used by Tishby)} \label{subBinning}
...
\begin{equation}
  I(T_{e,l}, X) = H(T_{e,l})
\end{equation}
See \autoref{figBinning} routine \ms{binning.estimateX}
...
\end{equation}
See \autoref{figBinning} routine \ms{binning.estimateY}
...
\end{equation}
See \autoref{figBinning} routine \ms{binning.Entropy} for implementation details
...
See \autoref{figBinning} routine \ms{binning.conditionalEntropy} for implementation
details
...
\begin{figure}[H]
    ...
    \end{pythonfigure}
    \caption{
      Implementation of binning MIE. Functions \ms{estimateX} and \ms{estimateY}
...
\autoref{figKDE} routines \ms{KDE.estimateX} and \ms{KDE.estimateY} for the
implementation.
...
\begin{figure}[H]
    ...
    \end{pythonfigure}
    \caption{
      Implementation of KDE MIE. Functions \ms{estimateX} and \ms{estimateY} are
...
deviate from the format assume that routines \ms{estimateX} and \ms{estimateY}
...
Notice that in our implementation methods \ms{estimateX} and \ms{estimateY} are
identical. This is due to the fact that our dataset \ms{X} no longer contains
...
\begin{itemize}
  \item{
      \ms{main_data.py} - is used to run experiments using Binning MIE or KDE MIE.

      run : \ms{python3 main_data.py --help}, for help
    }
  \item{
      \ms{main_as_if_random.py} - is used to run the Batching MIE with added Noise
      from the Binning MIE.

      run : \ms{python3 main_as_if_random.py --help}, for help
    }
\end{itemize}
\paragraph{And two auxiliary entry points}
\begin{itemize}
  \item{
      \ms{main_plot.py} - is to produce the visualization of the Information
      Plane.

      run : \ms{python3 main_plot.py --help}, for help
    }
  \item{
      \ms{main_movie.py} - is used to make a video of the Information Plane to
      help analyse the data.

      run : \ms{python3 main_movie.py --help}, for help
    }
\end{itemize}
